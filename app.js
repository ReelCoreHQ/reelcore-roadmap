...
const labels = {planned:'待实现', in_progress:'进行中', done:'已完成'};
...
function render() {
  ...
  for(const s of data.stages) {
    ...
    const selected=s.features.filter(f=>(status==='all'||f.status===status)&&`${f.id} ${f.title} ${f.acceptance}`.toLocaleLowerCase().includes(query));
    if(!selected.length) continue;
    visible+=selected.length;
    const section=el('section','stage'); const head=el('div','stage-head'); const title=el('div','stage-title');
    title.append(el('span','stage-id',s.id),el('h3','',s.title)); head.append(title,el('span','stage-count',`${s.features.filter(f=>f.status==='done').length} / ${s.features.length} 已完成`));section.append(head);
    for(const f of selected) {
      const row=el('details','feature'); row.id=f.id; const summary=el('summary');summary.append(el('span','feature-id',f.id),el('span','feature-title',f.title),el('span',`badge ${f.status}`,labels[f.status]),el('span','chevron','›'));row.append(summary);
      const detail=el('div','detail');detail.append(el('p','',f.acceptance));
...
async function init(){
 try {
  ...
  const features=data.stages.flatMap(s=>s.features);const done=features.filter(f=>f.status==='done').length;const percent=features.length?Math.round(done/features.length*100):0;
  $('total').textContent=features.length;$('done').textContent=done;$('active').textContent=features.filter(f=>f.status==='in_progress').length;$('percent').textContent=`${percent}%`;$('progress').style.width=`${percent}%`;$('scope').textContent=data.scope_note;
